...
###  Estrategia de solución 
Este ejercicio se caracterizó por el uso de una matriz bidimensional que hizo mucho más facil la implementación, se imprimir la matriz con un  "[ ]" si el asiento está vacio, si está ocupado imprime "[X]". Posteriormente hace la validación de: 1. Si el asiento está fuera de la matriz (asiento inexistente).2. Si el asiento está ocupado. Después está la función `void bucleReservas()` la cual hace que el programa siga en un bucle para guardar asientos.
